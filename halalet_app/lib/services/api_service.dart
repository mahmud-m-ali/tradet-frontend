...
class ApiService {
  ...
  static const String _defaultTunnelUrl = 'https://atomahmud.pythonanywhere.com/api';
  // ignore: unused_field
  ...
  void _checkResponse(http.Response response, String context) {
    if (response.statusCode == 401) {
      ...
      throw ApiException('Session expired', userMessage: 'Session expired. Please log in again.', statusCode: 401);
    }
    if (response.statusCode == 429) {
      throw ApiException('Rate limited', userMessage: 'Too many requests. Please wait a moment.', statusCode: 429);
    }
    if (response.statusCode >= 500) {
      throw ApiException('Server error', userMessage: 'Server error. Please try again later.', statusCode: response.statusCode);
    }
  }
  ...
  Future<Map<String, dynamic>> submitKyc({
    ...
    final response = await _post('/auth/kyc', body: {
      'id_type': idType,
      'id_number': idNumber,
      if (tradeLicenseNumber != null) 'trade_license_number': tradeLicenseNumber,
    });
  ...
  Future<User> getProfile() async {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to load profile');
    final data = jsonDecode(response.body);
  ...
  Future<List<Asset>> getAssets({
    ...
    try {
      ...
      if (response.statusCode != 200) throw ApiException('Failed to load assets');
  ...
  String executionType = 'market',
    ...
    final response = await _post('/trading/orders', body: {
      'asset_id': assetId,
  ...
  Future<List<OrderEvent>> getOrderEvents() async {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to load order events');
    final List<dynamic> data = jsonDecode(response.body);
    return data.map((j) => OrderEvent.fromJson(j)).toList();
  }

  Future<List<Order>> getOrders({String? status}) async {
    final path = status != null ? '/trading/orders?status=$status' : '/trading/orders';
    try {
      final response = await _get(path);
      if (response.statusCode != 200) throw ApiException('Failed to load orders');
      final List<dynamic> data = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> getPortfolio() async {
    try {
      ...
      if (response.statusCode != 200) throw ApiException('Failed to load portfolio');
      final data = jsonDecode(response.body);
      await CacheService.set('portfolio', data, ttl: const Duration(minutes: 5));
      return data;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> withdraw({
    ...
    final response = await _post('/wallet/withdraw', body: {
      'amount': amount,
      'bank_name': bankName,
  ...
  Future<List<Transaction>> getTransactions() async {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to load transactions');
    final data = jsonDecode(response.body);
  ...
  Future<List<PaymentMethod>> getPaymentMethods() async {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to load payment methods');
    final List<dynamic> data = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> addPaymentMethod({
    ...
    final response = await _post('/payment-methods', body: {
      'bank_name': bankName,
      'account_number': accountNumber,
  ...
  Future<List<Asset>> getWatchlist() async {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to load watchlist');
    final List<dynamic> data = jsonDecode(response.body);
    return data.map((j) => Asset.fromJson(j)).toList();
  }
  ...
  Future<Map<String, dynamic>> getComplianceInfo() async {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to load compliance');
    return jsonDecode(response.body);
  }
  ...
  Future<Map<String, dynamic>> createAlert({
    ...
    final response = await _post('/alerts', body: {
      'asset_id': assetId,
  ...
  Future<List<NewsArticle>> getNews({String? category, int limit = 30}) async {
    ...
    try {
      ...
      await CacheService.set('news_${category ?? "all"}', articles, ttl: const Duration(minutes: 15));
  ...
  Future<Map<String, dynamic>> calculateZakat({
    ...
    final response = await _post('/zakat/calculate', body: {
    ...
    if (response.statusCode != 200) throw ApiException('Failed to calculate Zakat');
    return jsonDecode(response.body);
  }
  ...
  Future<Map<String, ExchangeRate>> getExchangeRates() async {
    try {
      ...
      if (response.statusCode != 200) throw ApiException('Failed to load rates');
      final data = jsonDecode(response.body);
      final Map<String, dynamic> rates = data['rates'] ?? {};
      await CacheService.set('exchange_rates', rates, ttl: const Duration(hours: 1));
  ...
  Future<List<Map<String, dynamic>>> getChartHistory(
    ...
    final uri = Uri.parse('$baseUrl/market/history/$symbol').replace(
