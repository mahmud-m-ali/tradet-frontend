...
class _CandlestickChartState extends State<CandlestickChart> {
                ...
                ? const Center(
                    child: SizedBox(
                        ...
                        child: CircularProgressIndicator(
                            strokeWidth: 2, color: HalalEtTheme.accent)))
                : _activeData.isEmpty
                    ? const Center(
  ...
  Widget _infoBar(OhlcData d) {
    ...
    final color = change >= 0 ? HalalEtTheme.positive : HalalEtTheme.negative;
    return Row(
      children: [
        Text(d.date,
            style: const TextStyle(color: HalalEtTheme.textMuted, fontSize: 10)),
        const Spacer(),
        _infoChip('O', d.open),
        _infoChip('H', d.high),
        _infoChip('L', d.low),
      ...
      child: RichText(
        text: TextSpan(
          children: [
            TextSpan(
                text: '$label ',
                style: const TextStyle(color: HalalEtTheme.textMuted, fontSize: 9)),
            TextSpan(
  ...
  Widget _periodChip(String key, String label) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected
              ? HalalEtTheme.primaryLight.withValues(alpha: 0.2)
              : Colors.transparent,
        ...
        child: Text(label,
            style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.w600,
                color: selected ? HalalEtTheme.positive : HalalEtTheme.textMuted)),
      ),
    );
  }
